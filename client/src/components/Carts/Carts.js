...
export default function Carts({ cartShow, handleCartHide }) {
  ...
  if (data) {
    data.map((item) => {
      total += item.price.toFixed(3) * item.quantity;
      return null;
    });
    ...
    <div className={`carts-container ${cartShow && 'show-cart'}`}>
      ...
      <div className='carts-item-container'>
        <div className='carts-item'>
          {data &&
            data.map((item, index) => {
              return <Cart data={item} key={index} handleDelete={handleDelete} />;
            })}
        </div>
        {data && data.length && (
          <>
            ...
            <div className='carts-button-container'>
              <Link to={'/cart'}>
                <Button icon={<Show set='bold' primaryColor='royalblue' />} bordered color='primary' auto>
                  View Cart
                </Button>
              </Link>
            </div>
