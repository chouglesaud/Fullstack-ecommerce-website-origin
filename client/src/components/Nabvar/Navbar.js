...
export default function Navbar() {
  ...
  const searchProduct = (e) => {
    window.location = `${window.location.origin}/products?search=${search}`;
  };
    ...
    <>
      <div className='navbar-container' ref={navbar}>
        ...
        <div className='icon-container'>
          <input type='search' placeholder='Search' className='pa-2' onKeyUp={handleSearchProduct} />
          <button onClick={searchProduct} className='btn btn-small btn-primary' style={{ marginLeft: '5px' }}>
            Search
          </button>
          ...
          <AiOutlineUser className='nav-icon' onClick={() => setProfilShow(!profileShow)} />
        </div>
        <div className={`profile-popup-container ${profileShow && 'profile-popup-show'}`}>
