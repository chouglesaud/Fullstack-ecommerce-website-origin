...
export default function Product({ data }) {
  ...
  useEffect(() => {
    const isFavHandler = async () => {
      const res = await fetch(`${API_BASE_URL}/api/products/fav/${data.id}`, {
        headers: {
  ...
  return (
    <Link to={`/product/${data.id}`} className='lol'>
      <Atropos className='atropos-banner' shadow={true} rotateXMax={15} rotateYMax={15} shadowScale={1}>
        <Card hoverable clickable width='100%'>
          <Card.Body noPadding>
            <Card.Image objectFit='cover' autoResize={true} src={image[0]} width='100%' height={380} alt={data.name} />
          </Card.Body>
              ...
              <Col justify='space-between' color='primary' width='100%'>
                ...
                <Row justify='center'>
                  <Button.Group justify='center'>
                    ...
                    <Button
                      icon={<Heart set={`${isFav ? 'bold' : 'light'}`} primaryColor='white' />}
                      onClick={(e) => {
                        e.preventDefault();
