...
export default function SingleProductPage() {
  ...
  useEffect(() => {
    ...
    const isFavHandler = async () => {
      const res = await fetch(`${API_BASE_URL}/api/products/fav/${search}`, {
        headers: {
  ...
  const handleAdd = (type) => {
    ...
    type ? setProdQuantity(prodQuantity + 1) : setProdQuantity(prodQuantity - 1);
  };
              ...
              <div className='detail-content-container'>
                <h1>
                  ...
                  <span
                    ...
                    <Heart set={`${isFav ? 'bold' : 'light'}`} primaryColor='black' />
                  </span>{' '}
                </h1>
                <p className='detail-price'>₹{singleProduct.price}</p>
                <p className='detail-desc'>{desc}</p>
                {/* PRODUCT SIZE */}
                {singleProduct.gender !== 'Unisex' && (
                  <div className='product-size-container'>
                    <div className='size-button-container'>
                      <button className={`size-button-${curSize === 'M' && 'active'}`} onClick={() => setCurSize('M')}>
                        M
                      </button>
                      <button className={`size-button-${curSize === 'L' && 'active'}`} onClick={() => setCurSize('L')}>
                        L
                      </button>
                      <button className={`size-button-${curSize === 'X' && 'active'}`} onClick={() => setCurSize('X')}>
                        X
                      </button>
                      <button className={`size-button-${curSize === 'S' && 'active'}`} onClick={() => setCurSize('S')}>
                        S
                      </button>
                    </div>
                  </div>
