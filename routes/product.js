...
router.get('/:id', async (req, res) => {
  try {
    let product = await pool.query('SELECT * FROM products WHERE id = $1', [req.params.id]);
...
router.post('/fav/:id', verify, async (req, res) => {
  try {
    let fav = await pool.query('SELECT * FROM wishlist WHERE user_id = $1 AND product_id = $2', [
      req.user,
      req.params.id,
    ]);
    if (fav.rows.length === 0) {
      fav = await pool.query('INSERT INTO wishlist (user_id, product_id, is_fav) VALUES ($1,$2,$3)', [
        req.user,
        req.params.id,
        'true',
      ]);
      ...
      fav = await pool.query('UPDATE wishlist SET is_fav = $1 WHERE user_id = $2 AND product_id = $3 RETURNING *', [
        val,
        req.user,
        req.params.id,
      ]);
...
router.get('/fav/:id', verify, async (req, res) => {
  try {
    const isItFav = await pool.query('SELECT is_fav FROM wishlist WHERE user_id = $1 AND product_id = $2', [
      req.user,
      req.params.id,
    ]);
